	...
	public void test002() {
		...
		for (int i=1; i<=13; i++) {
			for (int j=1; j<=3; j++) {
				...
				int stepSequence = j;
				if (initList.parallelStream().filter(p -> p.getStep().equals(stepName) && p.getSequence() == stepSequence).count() == 0L) break;
				Long previousCount = 0L;
				Long initPreviousCount = 0L;
				Long nextCount = 0L;
				String previousStepName = "P_" + (i - 1);
				String nextStepName = "P_" + (i + 1);
				if (initList != null && initList.size() != 0) initPreviousCount = initList.parallelStream().filter(p -> p.getStep().equals(previousStepName)).count();
